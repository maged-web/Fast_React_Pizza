...
function CreateOrder() {
  ...
  const totalCartrice=useSelector(getTotalCartPrice);
  const priorityPrice= withPriority?totalCartrice*0.2:0;
  const totalPrice=totalCartrice+priorityPrice
  ...
  const {username,status:adressStatus,position,adress,error:errorAdress}=useSelector((state)=>state.user)
      ...
      <Form method="POST">
        ...
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center"> 
          ...
          <div className="grow">
            ...
            {formErrors?.phone && (
              <p className="text-xs mt-2 bg-red-100 text-red-700 rounded-md p-2">
                {formErrors.phone}
              </p>
            )}
          </div>
        ...
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center relative">
          ...
          <div className="grow">
            ...
            className="input w-full" disabled={isLoadingAdress} defaultValue={adress}/> 
            {adressStatus==='error' && (
              <p className="text-xs mt-2 bg-red-100 text-red-700 rounded-md p-2">
                {errorAdress}
              </p>
         ...
         <Button disabled={isLoadingAdress} type='small'
